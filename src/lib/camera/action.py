...
class Action:
    ...
    def get_last_photos(self, count=1):  # configs: List[(Config, Enum)]
        try:
            ...
            getfile = num_files if count == 1 else f"{start_range}-{end_range}"

            try:
                exec_in_dir(
                    self.dir,
                    gp,
                    [f"--folder={CANON_FOLDER}", f"--get-file={getfile}"],
                )
                self.log.action("DOWNLOADED", f"Last {count} photo(s)")
            except:
                raise
    ...
    def download_and_delete(self):
        dlcmd = [
            "--filename",
            f"{self.shoot.dir}{self.shoot.filename}",
            f"--folder={CANON_FOLDER}",
            "--get-all-files",
            "--delete-all-files",
        ]
    ...
    def capture(
        ...
        self.log.action("folder", f"{self.shoot.dir}/{self.shoot.filename}")
